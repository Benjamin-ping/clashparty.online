...
还未配置远程仓库，也未执行推送。网站反馈功能当前只在本机保存选择，不假装将反馈提交给维护者；需要真实反馈时，在 `site.json` 中填写 HTTPS 的 `feedbackUrl`（例如自己仓库的 Issues 表单地址）。

## Cloudflare Pages 部署

域名 DNS 在 Cloudflare 并不表示网站已部署，仍需创建 Pages 项目并绑定域名。

1. Cloudflare → Workers & Pages → 创建 Pages 项目 → 导入 GitHub 仓库。
2. 生产分支：`main`；框架预设：None。
3. 构建命令：`npm run build && npm run check`。
4. 输出目录：`dist`；Node 版本：22（可设置 `NODE_VERSION=22`）。
5. 首次部署成功后，在该 Pages 项目的「自定义域」添加 `clashparty.online`，按照平台提示核对 DNS。若域名已有站点记录，不要提前删除，先确认切换目标。
6. 确认 HTTPS、首页、深层教程地址、下载按钮和 404 返回正常，再对外使用。

文档依据：https://developers.cloudflare.com/pages/framework-guides/deploy-anything/
...
流程：抓取官方 Releases → 校验来源、版本与资产 → 合并历史 → 测试、构建、链接检查 → 有变化才提交 → Cloudflare Git 集成部署。
...
入门教程仅保留 Windows 11、Windows 7、Linux、Mac 四篇；Mac 芯片差异和 Linux 包格式在各自文章内说明。Windows 11 已接入维护者截图；Mac 有两张旧版 Mac 图和两张明确标注的 Windows 状态示意。Windows 7 已补齐 PowerShell 提示、WMF 下载、启动、订阅与节点测速图，并附微软官方更新步骤。四篇平台教程均包含先测试机场名称组、再测试 GLOBAL 的排查说明。安装与连接的进一步实机核验仍需补充，特别是 CentOS 具体版本兼容性。教程版本标记是编写基准，不代表已经执行全平台测试。机场推荐目前为 99吧与 FlyBit，资料统一维护在 content/services.json，已按用户指定页面整理套餐并确认跳转入口；价格以服务商结算页为准。

功能更新自动化：每次构建从同一份 releases.json 更新首页版本与摘要、下载中心、更新解读、功能更新页及相关手册/教程的「相关版本变化」。一条变化可同时关联多个功能；未知功能完整进入功能更新页，不遗漏。预发布仅保留在版本更新区，不进入稳定版手册补充。官方修订说明后，下次同步会重建相应内容，不重复追加。基础操作示例保留原版本，新增变化注明来源版本；不会生成未经操作的图片或把旧步骤冒充新版实测。检查通过后直接提交 main 并部署，不创建待审核 PR。


## GitHub 登录评论
文章底部接入 giscus，评论按 pathname 严格匹配，改标题不会丢失讨论。当前未配置仓库，显示评论暂未开放，不模拟提交成功。
